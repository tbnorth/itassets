...
class DependencyMapper:
    ...
    def known_id_prefix(self, asset, lookup, dependents):
        id_prefix = {v.prefix: v.description for v in self.ndef.ASSET_TYPE.values()}
    ...
    def make_parser():
        ...
        parser.add_argument("--output", help="Output folder", default="asset_inventory")
        parser.add_argument(
            "--theme",
    ...
    def propagate_dependent(self, assets, output="_dependent_types", field="type"):
    ...
    def report_to_html(self, asset, lookup, issues, title, write=True, dep_map=True):
        ...
        intermediates = [
            i for i in all_deps if i not in asset["_dependents"] and i not in finals
        ]
        ...
        if write:
            with open(OPT.output + "/" + self.html_filename(asset), "w") as rep:
                rep.write(html)
    ...
    def add_missing_deps(self, assets, other, ans):
        ...
        """
        for asset in assets:
            real_deps = [i for i in self.asset_dep_ids(asset) if not i.startswith("^")]
    ...
    def assets_to_dot(self, assets, issues, title, top):
        ...
        for asset in assets:
            ...
            for dep in sorted(
                ...
                ans.append(
                    f"  {other[dep]['_node_id']} -> {asset['_node_id']}" f"{edge_attr}"
                )
    ...
    def write_reports(self, assets, issues, title, archived):
        ...
        lookup = {i["id"]: i for i in assets}
        archived_listings = [
            self.report_to_html(i, lookup, issues, title, write=False, dep_map=False)
            for i in archived
        ]
        asset_types = []
    ...
    def write_map(self, base, assets, issues, title, leads_to, in_field, negate=False):
        ...
        with open(f"{OPT.output}/{base}.html", "w") as out:
            out.write(self.get_jinja().get_template("map.html").render(context))
